/**
 * You are given two 32-bit numbers, N and M, and two bit positions,
 * i and j. Write a method to insert M into N such that M starts at
 * bit j and ends at bit i. You can assume that the bits j through i
 * have enough space to fit all of M. That is, if M = 10011, you can
 * assume that there are at least 5 bits between j and i. You would
 * not, for example, have j = 3 and i = 2, because M could not fully
 * fit between bit 3 and bit 2.
 *
 * EXAMPLE
 * Input: N = 10000000000, M = 10011, i = 2, j = 6
 * Output: N = 10001001100
 *
 * Key:
 * First create mask 11110000011 on N, then place M into N.
 */
package bitmanipulation;

public class Insertion {
	
	public int insertion(int n, int m, int i, int j)
	{
		// create a binary of all 1s
		int allones = ~0;
		System.out.println(Integer.toBinaryString(allones));
		
		// create left part by shifting all ones by j+1
		int left = allones << (j+1);
		System.out.println(Integer.toBinaryString(left));
		
		// right by shifting 1 by 1
		int right = (1 << i)-1;
		System.out.println(Integer.toBinaryString(right));
		
		// create mask for n
		int mask = left | right;
		System.out.println(Integer.toBinaryString(mask));
		
		// clear n
		int n_cleared = n & mask;
		System.out.println(Integer.toBinaryString(n_cleared));
		
		// shift m by i places
		int m_moved = m << i;
		System.out.println(Integer.toBinaryString(m_moved));
		
		// set n, between i and j with m
		int updated = n_cleared | m_moved;
		System.out.println(Integer.toBinaryString(updated));
		
		// return set binary
		return updated;
	}
	
	public static void main(String[] args)
	{
		Insertion inst = new Insertion();
		System.out.println(Integer.toBinaryString(inst.insertion(1024, 19, 2, 6)));
	}

}
